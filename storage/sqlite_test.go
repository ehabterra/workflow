...
func TestSQLiteStorage_Basic(t *testing.T) {
	...
	if _, err := s.SaveState(context.Background(), "wf1", workflow.NewMarking(places), contextData, 0); err != nil {
		t.Fatalf("failed to save state: %v", err)
	}

	loadedMarking, loadedContext, _, err := s.LoadState(context.Background(), "wf1")
	if err != nil {
...
func TestSQLiteStorage_CustomFields(t *testing.T) {
	...
	if _, err := s.SaveState(context.Background(), "wf2", workflow.NewMarking(places), contextData, 0); err != nil {
		t.Fatalf("failed to save state: %v", err)
	}

	_, loadedContext, _, err := s.LoadState(context.Background(), "wf2")
	if err != nil {
		t.Fatalf("failed to load state: %v", err)
	}
...
func TestSQLiteStorage_DeleteState(t *testing.T) {
	...
	places := []workflow.Place{"draft"}
	contextData := map[string]any{}
	if _, err := s.SaveState(context.Background(), "wf3", workflow.NewMarking(places), contextData, 0); err != nil {
		t.Fatalf("failed to save state: %v", err)
	}
	if err := s.DeleteState(context.Background(), "wf3"); err != nil {
		t.Fatalf("failed to delete state: %v", err)
	}
	_, _, _, err = s.LoadState(context.Background(), "wf3")
	if err == nil {
...
func TestSQLiteStorage_EdgeCases(t *testing.T) {
	...
	_, err = s.SaveState(context.Background(), "empty_places", workflow.NewMarking(nil), nil, 0)
	if err != nil {
		t.Errorf("SaveState() with empty places failed: %v", err)
	}

	// Test saving with nil context
	_, err = s.SaveState(context.Background(), "nil_context", workflow.NewMarking([]workflow.Place{"draft"}), nil, 0)
	if err != nil {
		t.Errorf("SaveState() with nil context failed: %v", err)
	}

	// Test saving with empty context
	_, err = s.SaveState(context.Background(), "empty_context", workflow.NewMarking([]workflow.Place{"draft"}), map[string]any{}, 0)
	if err != nil {
		t.Errorf("SaveState() with empty context failed: %v", err)
	}

	// Test loading non-existent workflow
	_, _, _, err = s.LoadState(context.Background(), "non_existent")
	if err == nil {
	...
	_, err = s.SaveState(context.Background(), "multiple_places", workflow.NewMarking([]workflow.Place{"place1", "place2", "place3"}), nil, 0)
	if err != nil {
		t.Errorf("SaveState() with multiple places failed: %v", err)
	}

	m, _, _, err := s.LoadState(context.Background(), "multiple_places")
	if err != nil {
	...
	_, err = s.SaveState(context.Background(), "complex_context", workflow.NewMarking([]workflow.Place{"draft"}), complexContext, 0)
	if err != nil {
		t.Errorf("SaveState() with complex context failed: %v", err)
	}

	_, loadedContext, _, err := s.LoadState(context.Background(), "complex_context")
	if err != nil {
		t.Fatalf("LoadState() failed: %v", err)
	}
...
func TestSQLiteStorage_WithTable(t *testing.T) {
	...
	if _, err := s.SaveState(context.Background(), "wf_custom", workflow.NewMarking(places), contextData, 0); err != nil {
		t.Fatalf("failed to save state: %v", err)
	}

	loadedMarking, loadedContext, _, err := s.LoadState(context.Background(), "wf_custom")
	if err != nil {
		t.Fatalf("failed to load state: %v", err)
	}
	loadedPlaces := loadedMarking.Places()
...
func TestSQLiteStorage_WithCustomFields(t *testing.T) {
	...
	if _, err := s.SaveState(context.Background(), "wf_custom_fields", workflow.NewMarking([]workflow.Place{"draft"}), contextData, 0); err != nil {
		t.Fatalf("failed to save state: %v", err)
	}

	_, loadedContext, _, err := s.LoadState(context.Background(), "wf_custom_fields")
	if err != nil {
		t.Fatalf("failed to load state: %v", err)
	}
