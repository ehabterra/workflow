...
//
// Initialize creates the table only; it does not migrate a pre-existing table
// or create the due index. Use EnsureSchema for the M4 due index (ListDue /
// Manager.FireDue), which is idempotent on both fresh and pre-existing tables.
func Initialize(db *sql.DB, schema string) error {
	_, err := db.Exec(schema)
	return err
}

// SaveState saves the workflow's current places and any configured custom fields
// from its context. It does not maintain the due index (the due column is left
// untouched); for a timed definition, save through the Manager or
// SaveVersionedStateWithDue so the index stays consistent.
...
// SaveVersionedStateWithDue or go through the Manager so the index stays current.
...
// committed atomically. See RunInTx. Like SaveVersionedState it does not maintain
...
func (s *SQLiteStorage) saveVersionedState(ctx context.Context, q querier, id string, marking workflow.Marking, ctxData map[string]any, expectedVersion int64, due *time.Time, setDue bool) (int64, error) {
	stateJSON, err := json.Marshal(marking)
	if err != nil {
