...
func TestVersionedStorage_CreateAndIncrement(t *testing.T) {
	...
	v, err := store.SaveVersionedState(ctx, "wf", []workflow.Place{"a"}, nil, 0)
	if err != nil {
	...
	v, err = store.SaveVersionedState(ctx, "wf", []workflow.Place{"b"}, nil, 1)
	if err != nil {
...
func TestVersionedStorage_CreateConflictWhenExists(t *testing.T) {
	...
	if _, err := store.SaveVersionedState(ctx, "wf", []workflow.Place{"a"}, nil, 0); err != nil {
		t.Fatalf("first create: %v", err)
	}
	// Second create (expected 0) on an existing id must conflict.
	if _, err := store.SaveVersionedState(ctx, "wf", []workflow.Place{"x"}, nil, 0); !errors.Is(err, workflow.ErrConflict) {
...
func TestVersionedStorage_StaleUpdateConflicts(t *testing.T) {
	...
	if _, err := store.SaveVersionedState(ctx, "wf", []workflow.Place{"b"}, nil, 1); err != nil {
		t.Fatalf("writer1: %v", err)
	}
	// Writer 2 still thinks version is 1 -> stale -> conflict.
	if _, err := store.SaveVersionedState(ctx, "wf", []workflow.Place{"c"}, nil, 1); !errors.Is(err, workflow.ErrConflict) {
		t.Fatalf("writer2 err = %v, want ErrConflict", err)
	}
	// State reflects only writer 1's change.
	places, _, v, err := store.LoadVersionedState(ctx, "wf")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
