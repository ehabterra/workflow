...
func NewServer(app *App) (*Server, error) {
	...
	s.mux.HandleFunc("GET /diagrams", s.diagrams)
...
func (s *Server) expenseDetail(w http.ResponseWriter, r *http.Request) {
	...
	diagram, err := s.app.ExpenseDiagram(r.Context(), view.ID)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// diagrams renders both nets as live Mermaid diagrams — the same structures
// the engine executes, generated by Workflow.Diagram (no hand-drawn drift).
func (s *Server) diagrams(w http.ResponseWriter, r *http.Request) {
	expense, payment, err := s.app.NetDiagrams()
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
