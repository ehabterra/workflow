...
| M2.5 | **CPN in YAML** — `initial_tokens` key seeds colored tokens; `ClearPlace`. | ✅ | Retired the aspirational `cpn_enabled`/`token_schemas` stubs (they don't fit the unified model); strict decoding still rejects unknown keys. |
